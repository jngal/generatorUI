 ...
 * @property {string} type - Field type - "string", "boolean", "enum", "date", "time", "datetime", "text", "numeric", "container", "files", "password"
 ...
 * @property {bool} recommended - Recommended
 * @property {element} customComponent - Component displayed after the field.
 * @param {bool} validationIcon - Show validation icon?
 */
...
class Detail extends React.Component {
  ...
  buildInput = x => {
    switch (x.type) {
      ...
      //       ref={`${x.field}_ref`}
      //       tabIndex={x.tabIndex}
      //       onBlur={(e) => this.props.handleBlur(x, null)}
      //       autoUpload={x.autoUpload}
      //       multiple={x.multiple}
      //       requiredMaximumSize={x.requiredMaximumSize}
      //     />
      //   );
      ...
      case 'datetime':
        return (
          ...
          <DateTimePicker
            name={x.field}
            onChange={this.props.onChange}
            value={ this.props.getValue(x.field) instanceof Date ? this.props.getValue(x.field) : new Date()}
            disabled={x.disabled || this.props.disabled}
            min={x.min}
            max={x.max}
            format={x.format}
      ...
      case 'autocomplete':
        return (
          <AutocompleteComponent
            ...
            tabIndex={x.tabIndex}
            isLoading={this.props.parentProps[x.data] ? this.props.parentProps[x.data].isLoading : false}
            onBlur={(value) => this.props.handleBlur(x, value)}
            valueField={x.valueField}
          />
        );
    ...
    return (
      <React.Fragment key={x.field}>
        <div className={x.required && !x.disabled ? `k-form-field required form-${x.type} form-field-${x.field} ${customClass}` : `k-form-field form-${x.type} form-field-${x.field} ${disabledClass} ${customClass}`}>
          <span>
            {x.validationIcon && <span className={`k-icon validation-image`}></span>}
            {x.title}
          </span>
          {this.buildInput(x)}
          {x.tooltip &&
  ...
  buildPageClassName = pageIndex => {
    if (pageIndex > 0) {
      if (this.state.expandedPageIndex === pageIndex) {
        const scrollableElement = document.getElementsByClassName("scrollable-navbar")[0];
        return `sidebar-${pageIndex} col-4 ${this.props.sidebarHorizontal ? 'sidebar-horizontal' : ''} ${scrollableElement && scrollableElement.scrollHeight > scrollableElement.clientHeight ? '' : 'scrollable-expanded-page'}`;
      }
      else {
