...
class DialogFilterForm extends React.Component {
  ...
  operatorSK = (item) => {
    ...
    if (item.filter === 'boolean') {
      value = this.state.input[item.field + 'Value'] === false ? false : (this.state.input[item.field + 'Value'] ? true : null)
    }
    else {
      value = this.state.input[item.field + 'Operator'] ? this.state.input[item.field + 'Operator'] : null
    }
    const operator = FILTER_OPERATORS[item.filter && item.filter !== "none" ? item.filter : 'text'];
    return operator ? operator.find(item => item.operator === value) : undefined;
  }
  ...
  render() {
    return (
      <div ref={this.ref} className="k-form-inline dialogFilter">
            ...
            case "numeric":
              ...
              return (
                <div className="k-form-field" key={item.field}>
                  ...
                  <div>
                    <ExtendedDropDownList
                      name={item.field + '.' + item.textField + 'Operator'}
                  ...
                  <Input
                    name={item.field + '.' + item.textField + 'Value'}
                    onChange={this.handleChange}
                    value={this.state.input[item.field + '.' + item.textField + 'Value'] || ""}
                  />
                </div>
              )    
