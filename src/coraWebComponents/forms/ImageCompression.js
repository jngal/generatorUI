...
class ImageCompression extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      ratio: 100.0,
      file: props.file,
      maxSize: props.requiredMaximumSize
        ? stringToBytes(props.requiredMaximumSize)
        : null,
  ...
  resize = (src) => {
    ...
    image.onload = () => {
      ...
      const name = this.state.outputFile
        ? this.state.outputFile.name
      ...
      this.setState(
        {
          file: f,
          ratio:
            file.size === 0 && this.state.maxSize
              ? 100
              : (this.state.maxSize / file.size) * 100,
        },
        () => {
          this.resize(f.data);
        }
  ...
  onInputTextChange = (name, value) => {
    this.setState(
      {
        outputFile: new File([this.state.outputFile], value, {
      ...
      () => {
        if (this.props.file) {
          this.props.onChange(name, this.state.outputFile);
        }
      }
    );
  };
  ...
  saveFile = () => {
    ...
    if (this.props.requiredMaximumSize && this.state.outputFile)
      if (this.state.outputFile.size > this.state.maxSize)
        errorMessage = `Pre uloženie súboru je potrebná výsledná veľkosť menej ako ${bytesToSize(
          this.state.maxSize
        )} !`;

    this.setState({
      errorMessage,
      validMessage: errorMessage === "" ? "Súbor uložený" : "",
    });
  ...
  render() {
    const { file, outputFile, ratio, errorMessage, validMessage } = this.state;
    ...
    return (
      <div className="image-compress-container">
          ...
          <table className="table table-bordered">
            <thead>
              <tr>
                ...
                <th>
                  Veľkosť po: {outputFile ? bytesToSize(outputFile.size) : ""}
                </th>
              </tr>
            </thead>
          </table>
        ...
        {!this.props.file && (
          <Button
            disabled={!file || !outputFile || disabled}
            onClick={this.saveFile}
            label="Uložiť"
            primary
          />
        )}
        {!this.props.file && errorMessage && (
          <span className="message error">{errorMessage}</span>
        )}
        {!this.props.file && validMessage && (
          <span className="message valid">{validMessage}</span>
        )}
      </div>
    );
  }
...
ImageCompression.propTypes = {
  ...
  requiredMaximumSize: PropTypes.string.isRequired,
